...
export const findValueOfADollarByState = function(state) {
    return colMapping[state].dollar;
};

export const calculatePayAfterCostOfLiving = function(state, afterTaxIncome){
    let multiplier = findValueOfADollarByState(state);
    const costOfLivingAdjPay = (afterTaxIncome * multiplier).toFixed(2);
    return costOfLivingAdjPay;
};

export const displayMedianRentByState = function(state) {
    return colMapping[state].rent;
}
